...
export async function processCommit(repoFullName, commitData, eventId = null) {
  try {
    ...
    if (repo.event.discordChannelId) {
      try {
        const { notifyNewCommit } = await import('../bot/notifications.js');
        await notifyNewCommit(
          repo.event.discordChannelId,
          commit,
          { name: repo.user.name, _id: repo.user._id },
          { repoName: repo.repoName }
        );
        console.log('✅ Discord notification sent');
      } catch (notifError) {
        console.error('⚠️ Discord notification failed:', notifError.message);
      }
    }
